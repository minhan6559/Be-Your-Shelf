 ...
 * Manages the process of validating payment details, processing payments, saving orders, and clearing the shopping cart.
 */
public class PaymentController {
    ...
    private void updateTotalAmountLabel() {
        totalAmountLabel.setText("Total Amount: $" + String.format("%.2f", totalAmount));  // Format and display total amount
    }

    /**
     * Handles the payment process when the user submits their payment details.
     * Validates the payment details, processes the payment, saves the order, and clears the cart if successful.
     */
    @FXML
    public void handlePayment() {
        if (!validatePaymentDetails()) return;  // Validate payment details before proceeding
        ...
        try {
            ...
            UIUtils.showAlert("Payment Successful", "Your payment was successful! Order Reference: " + orderReference);  // Notify user of success
    ...
    private String processPaymentDetails() throws SQLException {
        ...
        // Process payment and return order reference; throw SQLException if the payment fails
    ...
    private void saveOrder(String orderReference) throws SQLException {
        ...
        // Place the order using the order service; throw SQLException if the order saving fails
    ...
    private void clearCartAfterPayment() throws SQLException {
        ...
        // Remove selected books from the database and clear them from the in-memory cart
    ...
    private void handlePaymentError() {
        UIUtils.showError("Payment Failed", "An error occurred during payment. Please try again.");  // Display error message
