 ...
 * Handles displaying cart items, quantity adjustment, checkout, and stock management.
 */
public class ShoppingCartController {
    ...
    private final IInventoryService inventoryService = ServiceManager.getInstance().getInventoryService();  // Inventory service
    ...
    public void initialize() {
        ...
        quantityColumn.setCellFactory(_ -> new TableCell<>() {
            ...
            private final HBox container = new HBox(10, decrementButton, quantityLabel, incrementButton);  // Add spacing between elements

            {
                // Align elements in the center of the HBox
                container.setStyle("-fx-alignment: center;");
                container.setSpacing(10);  // Add spacing between the buttons and label
            }
            ...
            protected void updateItem(Integer quantity, boolean empty) {
                ...
                if (empty) {
                    ...
                    CartTableItem cartItem = getTableView().getItems().get(getIndex());
                    quantityLabel.setText(String.valueOf(cartItem.getQuantity()));  // Set the quantity label
    ...
    private void loadStylesheet() {
        ...
        UIUtils.loadCSS(root, "/com/thereadingroom/css/table-style.css");
    }
    ...
    private void loadCartItems() {
        List<CartTableItem> cartItems = shoppingCart.getBooks().entrySet().stream()
                .map(entry -> new CartTableItem(entry.getKey(), entry.getValue()))  // Convert each book and its quantity into a CartTableItem
    ...
    /**
     * Handles the checkout process by confirming and reserving stock for selected items.
     */
    @FXML
    public void handleCheckout() {
        ...
        if (confirmed) {
            ...
            loadPaymentScreen(selectedItems.stream().mapToDouble(CartTableItem::getTotalAmount).sum());  // Load payment screen with total amount
        }
    }

    /**
     * Reserves stock for the selected items in memory before proceeding with payment.
     *
    ...
    private void loadPaymentScreen(double totalAmount) {
        ...
        UIUtils.loadModal("/com/thereadingroom/fxml/user/payment.fxml", "Payment", controller -> {
            PaymentController paymentController = (PaymentController) controller;
            int userId = SessionManager.getInstance().getUserId();
            paymentController.setPaymentDetails(totalAmount, userId, shoppingCart, this);  // Pass payment details to the PaymentController
        }, paymentStage);
    }

    /**
     * Finalizes the stock adjustments and clears the cart after a successful payment.
     */
    ...
    /**
     * Reverts any stock that was reserved in memory in case of payment failure or cancellation.
     */
    ...
    private void removeItem(CartTableItem cartItem) {
        System.out.println("Removing book from cart - Cart ID: " + shoppingCart.getCartId() + ", Book ID: " + cartItem.getBook().getBookId());
        shoppingCart.removeBook(cartItem.getBook());  // Remove from the cart model in memory
        cartService.removeBookFromCart(shoppingCart.getCartId(), cartItem.getBook().getBookId());  // Remove from the database
     ...
     * @param adjustment The quantity adjustment (positive for increment, negative for decrement).
     */
    private void adjustQuantity(CartTableItem cartItem, int adjustment) {
        ...
        if (newQuantity > 0) {
            ...
            cartService.updateBookQuantity(shoppingCart.getCartId(), cartItem.getBook().getBookId(), newQuantity);  // Update the database
